...
def blog_list_common_command(request, blogs_all_list):
    paginator = Paginator(blogs_all_list, 5)  # 10
    ...
    context['blog_types'] = BlogType.objects.annotate(blog_count=Count("blog"))  # annotate拓展查询
...
def blog_list(request):
    ...
    context = blog_list_common_command(request, blogs_all_list)
    return render(request, 'blog/blog_list.html', context)


def blog_detail(request, blog_pk):
    blog = get_object_or_404(Blog, pk=blog_pk)
    read_cookie_key = read_statistics_once_read(request, blog)  # read_statistics中导入的read_statistics_once_read方法
    ...
    response.set_cookie(read_cookie_key, 'true', max_age=60)  # 设置cookie
    return response
...
def blogs_with_date(request, year, month):
    ...
    context = blog_list_common_command(request, blogs_all_list)
    context['blogs_with_date'] = '%s年%s月' % (year, month)  # 标题
